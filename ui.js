 ...
 * 職責：雙色命名渲染、戰鬥特效、日誌過濾、手動技能 CD 動畫
 */
...
UIManager.prototype.showDamage = function(dmg, isCrit) {
    var picContainer = document.getElementById('monster-pic');
    if (!picContainer) return;

    var parent = picContainer.parentElement;
...
UIManager.prototype.switchLog = function(type, event) {
    ...
    for (var j = 0; j < logs.length; j++) {
        ...
        if (type === 'all') {
            ...
            // 檢查日誌內容的 class 是否符合 (combat 或 loot)
...
UIManager.prototype.updateHPs = function(player, monster) {
    ...
    if (pBar && pTxt) {
        var pPer = (player.battle.hp / player.battle.maxHp) * 100;
        pBar.style.width = pPer + "%";
        pTxt.innerText = Math.floor(player.battle.hp) + " / " + player.battle.maxHp;
    }
    
    if (mBar && mTxt && monster) {
        var mPer = (monster.hp / monster.maxHp) * 100;
        mBar.style.width = mPer + "%";
...
UIManager.prototype.renderBag = function() {
    ...
    for (var i = 0; i < p.data.bag.length; i++) {
        ...
        (function(idx, itm) {
            ...
            var icon = (itm.itemType === 'equip') ? (itm.type === 'weapon' ? "⚔️" : "🛡️") : 
                         (itm.itemType === 'scroll' ? "📜" : "💎");
            
            slot.innerHTML = '<div class="item-icon">' + icon + '</div>' +
                             '<div class="item-info">' +
                             '<div class="item-name">' + self.getItemNameHTML(itm) + ' ' + (itm.count > 1 ? 'x' + itm.count : '') + '</div>' +
                             '</div>';
...
UIManager.prototype.renderStats = function() {
    ...
    if (statBox) {
        ...
        if (p.data.lv >= nextBreak) {
            ...
            bBtn.innerText = "突破至 " + (GAME_DATA.REALMS[p.data.realmIdx + 1] || "更高境界");
            bBtn.onclick = function() { p.breakthrough(); };
            ...
            for (var k in sMap) {
                (function(key) {
                    ...
                    row.innerHTML = '<span>' + sMap[key] + '(' + p.data.baseStats[key] + ')</span><button onclick="_X_CORE.addStat(\'' + key + '\')">+</button>';
    ...
    var eSlot = document.getElementById('equipment-slots');
    if (eSlot) {
        eSlot.innerHTML = "";
        var slots = ['weapon', 'body'];
        for (var j = 0; j < slots.length; j++) {
            (function(sName) {
                var itm = p.data.equips[sName];
                var div = document.createElement('div');
                div.className = "item-slot";
                div.style.marginBottom = "5px";
                div.innerHTML = itm ? '<div class="item-icon">' + (sName === 'weapon' ? '⚔️' : '🛡️') + '</div>' + self.getItemNameHTML(itm) : 
                                      '<div style="color:#444">未穿戴' + (sName === 'weapon' ? '法寶' : '護甲') + '</div>';
                if (itm) div.onclick = function() { self.core.inventory.showItemDetail(sName, true); };
...
UIManager.prototype.renderSkills = function() {
    var self = this;
    var p = this.core.player;
    var list = document.getElementById('learned-skills-list');
    if (!list) return;
    list.innerHTML = "";
    ...
    for (var i = 0; i < p.data.learnedSkills.length; i++) {
        (function(sId) {
            ...
            card.className = 'skill-card ' + (isEq ? 'equipped' : '');
...
UIManager.prototype.showSkillEquipMenu = function(sId) {
    ...
    this.showModal("裝備神通", s.desc, "放入一號位", function() {
        ...
        // 移除已存在的相同技能
        for (var i = 0; i < p.data.skills.length; i++) {
            if (p.data.skills[i] === sId) p.data.skills[i] = null;
        }
        p.data.skills[0] = sId; 
        p.save();
        self.renderAll();
...
UIManager.prototype.showModal = function(t, b, bt, bf, mf) {
    ...
    var ab = document.getElementById('modal-action-btn');
    ab.innerText = bt;
    ...
    if (mf) {
        ...
        mb.onclick = function() { mf(); _X_CORE.ui.closeModal(); };
    } else {
