class XianXiaGame {
    constructor() {
        ...
        this.state = saved || {
            ...
            bag: [], eq: { weapon: null, body: null }, lastSave: Date.now()
        };
        this.curHp = 0;
        this.realms = ["凡人", "練氣期", "築基期", "金丹期", "元嬰期", "化神期", "煉虛期", "合體期", "大乘期", "渡劫期"];
        this.jobNames = { sword: "劍修", body: "體修", soul: "靈修" };
        this.prefixes = [
            { n: "迅捷的", stat: "agi", bonus: 1.2 }, { n: "強力的", stat: "str", bonus: 1.2 },
            { n: "厚重的", stat: "vit", bonus: 1.2 }, { n: "聰慧的", stat: "int", bonus: 1.2 },
            { n: "暴虐的", stat: "critDmg", bonus: 0.5 }, { n: "仙靈的", stat: "all", bonus: 1.1 }
        ];
        this.rt = { auto: false, lastAuto: 0, skillCD: 0, skillMaxCD: 8000, lastRegen: Date.now() };
        this.m = { n: "小妖", hp: 50, mx: 50, exp: 20, elite: false };
        
        try { this.init(); } catch(e) { console.error("初始化發生錯誤，但遊戲嘗試繼續:", e); }
    }

    // 終極安全更新函數：絕對不會噴 Cannot set properties of null
    ...
    loop() {
        ...
        if (this.rt.skillCD > 0) {
            this.rt.skillCD -= 100;
            this.u('skill-cd', Math.max(0, (this.rt.skillCD / this.rt.skillMaxCD) * 100) + "%", true);
        } else {
            const btn = document.getElementById('btn-skill'); if (btn) btn.disabled = false;
        }
    }
    ...
    atk(isManual, x, y, multi = 1) {
        ...
        if (this.m.hp <= 0) this.onDie(); else if (!isManual || Math.random() < 0.3) this.monsterCounterAtk();
        this.update();
    }
    monsterCounterAtk() {
        if (Math.random() * 100 < this.evasion) { this.pop("閃避", false, 120, 180); return; }
        this.curHp -= Math.floor(this.m.mx * 0.08);
        if (this.curHp <= 0) { this.curHp = Math.floor(this.finalHp * 0.2); this.rt.auto = false; }
        this.update();
    }
    spawn() {
    ...
    onDie() { this.gainXp(this.m.exp); if (Math.random() < (this.m.elite ? 0.8 : 0.25)) this.drop(); this.spawn(); }
    ...
    drop() {
        const r = Math.random(); let q = r < 0.01 ? 4 : r < 0.05 ? 3 : r < 0.2 ? 2 : r < 0.5 ? 1 : 0;
        const type = Math.random() < 0.5 ? 'weapon' : 'body';
        const qNames = ["凡品", "良品", "精品", "極品", "仙品"];
        const tNames = type === 'weapon' ? ["長劍", "重錘", "靈珠", "唐刀"] : ["布袍", "輕甲", "重鎧", "法衣"];
        let baseVal = (5 + this.state.p.lv * 2) * [1, 1.5, 2.5, 4, 8][q];
        let pfx = ""; if (q >= 2 && Math.random() < 0.5) { const p = this.prefixes[Math.floor(Math.random() * this.prefixes.length)]; pfx = p.n + " · "; baseVal *= 1.2; }
        const item = { id: Date.now(), type, q, val: Math.floor(baseVal), lvReq: Math.max(1, this.state.p.lv + Math.floor(Math.random()*3)-1), name: pfx + qNames[q] + "·" + tNames[Math.floor(Math.random()*4)] };
        if (this.state.bag.length < 20) this.state.bag.push(item);
    }
    equip(id) {
    ...
    renderStats() {
        ...
        if (el) el.innerHTML = Object.entries(map).map(([k, n]) => `<div class="stat-row"><span>${n} (<b>${this.state.p[k]}</b>)</span><button class="btn-plus" onclick="game.addStat('${k}')">+</button></div>`).join('');
    }
    renderBag() {
        const list = document.getElementById('bag-list'); if (!list) return;
        list.innerHTML = this.state.bag.map(i => `<div class="item-card quality-${i.q}"><div style="display:flex; justify-content:space-between;"><b>${i.name}</b><span style="font-size:10px; color:${this.state.p.lv >= (i.lvReq||1) ? 'var(--success)' : 'var(--danger)'}">Lv.${i.lvReq||1}</span></div><div style="margin:5px 0; color:#8b949e;">${i.type==='weapon'?'ATK':'HP'} +${i.val}</div><button class="btn-small" onclick="game.equip(${i.id})">穿戴</button></div>`).join('');
        this.u('bag-count', this.state.bag.length);
    }
    ...
    switchTab(tab, el) {
        ...
        const target = document.getElementById('p-' + tab); if (target) target.style.display = 'flex';
        if (el) el.classList.add('active'); if (tab === 'bag') this.renderBag();
    }
    showClassModal() { const m = document.getElementById('class-modal'); if (m) m.style.display = 'flex'; }
    chooseClass(j) { this.state.p.job = j; const m = document.getElementById('class-modal'); if (m) m.style.display = 'none'; this.calc(); this.update(); this.save(); }
