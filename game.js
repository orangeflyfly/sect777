...
class XianXiaGame {
    ...
    renderStats() {
        ...
        el.innerHTML = Object.entries(map).map(([k, n]) => `<div class="stat-row"><span>${n} (<b>${this.state.p[k]}</b>)</span><button class="btn-plus" onclick="game.addStat('${k}')">+</button></div>`).join('');
    }
    ...
    respec() { if (!confirm("重置所有點數？")) return; const p = this.state.p; p.pts += (p.str-5)+(p.vit-5)+(p.agi-5)+(p.int-5); p.str=5; p.vit=5; p.agi=5; p.int=5; this.calc(); this.curHp=this.finalHp; this.renderStats(); this.update(); this.save(); }
